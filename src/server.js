...
// app.get('*', routeCache.cacheSeconds(0), async (req, res, next) => {
  ...
  try {
    ...
    if(isAdmin){
      ...
      html = html.replace('body', `<!-- Global site tag (gtag.js) - Google Analytics --> <script async src="https://www.googletagmanager.com/gtag/js?id=UA-125798720-1"></script> <script> window.dataLayer = window.dataLayer || []; function gtag(){dataLayer.push(arguments);} gtag('js', new Date()); gtag('config', 'UA-125798720-1'); </script>`);
      res.send(`<!doctype html>${html}`);
    }
  } catch (err) {
    next(err);
